...
def hj_query(word):
    html=pq('http://dict.hjenglish.com/jp/jc/'+word, headers={'user-agent':'Mozilla/5.0 (Windows NT 6.3; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.95 Safari/537.36'})
    ...
    for i in range(1,21):
        ...
        if i==1:
            ...
            cprint('-----'*16,'white')
        cprint(chr(i+64)+'  '+kannji+' '+hiragana+' '+english,'magenta')
        ...
        for li in yakus.items():
            ...
            if tatoes!=None:
                ...
                tatoes=re.sub(r'</b>','',tatoes)
                if tatoes[0:4]=='<div':   #優しい
                    tatoes=re.sub(r'<\s+/script>','</script>',tatoes)
                    tatoes=re.sub(r'<\s+/span>','</span>',tatoes)
                    tatoes=re.sub(r'<\s+/div>','</div>',tatoes)
                    tatoes=pq(tatoes)
                    tatoes=tatoes.text()
                    tatoes=re.sub(r'GetTTSVoice(.*?);','\n ',tatoes)
                tatoes=re.sub(r'/','\n  ',tatoes)
                if tatoes[0]=="\n":
                    tatoes=tatoes[1:]
                print tatoes
